...
function init(module, app, next) {
  ...
  calipso.lib.step(
    ...
    function done() {
      ...
      // Add dynamic helpers
      calipso.helpers.addHelper('getContent', function () {
        return getContent;
      });

      // Get content list helper
      calipso.helpers.addHelper('getContentList', function () {
...
function getForm(req, action, title, contentType, next) {
  ...
  ContentType.findOne({contentType:contentType}, function (err, ct) {
    ...
    // Add any fields
    if (!err && ct && ct.get("fields")) {
      // FIX as this added later, get is 'safer' if not existing in document
...
function showAliasedContent(req, res, template, block, next) {
  ...
  // Check type
  if (calipso.lib._.any(allowedFormats, function (value) {
    return value === format;
  })) {
